...
try:
    ...
    from app.charts import top_spenders_bar, free_vs_paid_chart, fee_timeline
...
tab_player, tab_club, tab_spenders, tab_freepaid, tab_agefee = section_tabs(
    ["Player History", "Club Summary", "Top Spenders", "Free vs Paid", "Age vs Fee"],
    key="transfers_tabs",
)
        ...
        if sel_season and sel_season != "-":
            ...
            if inc_df.empty and out_df.empty:
                ...
                try:
                    ...
                    hist_df = _pd.concat(
                        [df for df in [inc_df, out_df] if not df.empty],
                        ignore_index=True,
                    )
                except Exception:
                ...
                if "transfer_fee" in hist_df.columns:
                    hist_df["transfer_fee"] = hist_df["transfer_fee"].apply(
                        _fmt_currency
                    )
...
with tab_spenders:
    ...
    ov_season, ov_competition = filter_bar(
        include_season=True, include_competition=True, key_prefix="transfers_spenders"
    )
    if ov_season and ov_competition:
        ...
        spenders_df = df_from_list(
            get_list(spenders) or get_list(spenders, key="spenders")
        )
        if spenders_df.empty:
            empty_state("No spenders data.")
        else:
            st.plotly_chart(
                top_spenders_bar(spenders_df),
                use_container_width=True,
                key="transfers_top_spenders",
            )
            show_cols = [
                c
                for c in [
...
with tab_freepaid:
    ...
    fp_season, fp_competition = filter_bar(
        include_season=True, include_competition=True, key_prefix="transfers_freepaid"
    )
    if fp_season and fp_competition:
        api_season_fp = _to_long_season(fp_season)
        free_paid = api.transfers_free_vs_paid(api_season_fp, fp_competition) or {}
...
with tab_agefee:
    ...
    if age_fee_df.empty:
        ...
        try:
            ...
            st.dataframe(
                disp.style.format(fm), use_container_width=True, hide_index=True
            )
        except Exception:
            if "avg_transfer_fee" in disp.columns:
